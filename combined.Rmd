...
This match can be highly competitive. There are countless strategies that can be used to out maneuver the opposing team. Each champion comes with a learning curve that separates the best players from the millions of players around the world.
...
```{r minutes}
lol <- lol %>%
  mutate("durationMin" = lol$matchDuration / 60)
...
Now, we should note the spike at the start of the x-axis (that is, the matchs that were finished in less than 10 minutes). These are outliers that represent matchs where one team absolutely destroyed the other, or one team went AFK (away from keyboard, or inactive).
...
In accordance with our assumptions, the team that achieves first blood has a higher chance of winning. Killing an enemy early in the match allows a team to set up a stronger stance throughout the match, as the enemy team now has only four players to defend their assets and nexus.
...
Indeed, we are right in our assumptions! By destroying an enemy tower first, a team can develop a strong position early in the match, and offer more passage for minions and teammates in the path to the enemy nexus. 
...
Correct! And by what a margin! Destroying an enemy team's inhibitor can be a major turning point during the match. It allows the team to spawn super minions that act as a nuisance for the enemy team, which in turn can lead to a chain reaction of favorable events.

### First Baron

The *baron nashor* is a powerful neutral enemy within the arena of Summoner's Rift; it is neutral because it favors neither team, and can be considered a part of its own team! Killing it will give a team special power-ups for a long duration of the match. So it is expected that a team killing the baron first will have an edge, right?
...
Finally, we will look at the *Rift Herald*, which is another monster within the arena. Killing it also gives a team benefits; it is important to note that this monster automatically despawns at around 20 minutes into the match (which means a team must kill it beforehand).
...
Woah! Remember that I said the Rift Herald despawns 20 minutes into the match. There are more instances where neither team defeats the rift herals, but the matchs in which it is defeated, the slaying team tends to be victorious.
...
Machine learning is powerful as it can take large amounts of data and make predictions on the FUTURE! Fitting a machine learning model gives us a tool to gain insights on what has not happened yet based on current stats on a match. For example, we can train a ML model to predict which team will win based off of which team captures objectives first. The simplest machine learning strategy is linear regression! Fitting this model essentially gives us a line of best fit for the data. 
